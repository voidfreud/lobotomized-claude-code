...
Electron apps have a window; a headless agent can't see one. So the deliverable isn't a markdown file saying "\`npm start\` opens a window" — it's a **driver script** that launches the app under xvfb, exposes a REPL (click, type, screenshot), and lets an agent poke the UI by sending lines of text. \`SKILL.md\` then becomes a short manual for that driver.
...
\`--no-sandbox\` is almost always needed in containers — Electron's sandbox needs CAP_SYS_ADMIN or user namespaces, neither present by default.

## Step 2 — build the REPL driver

Once it launches, turn that throwaway script into a REPL. Start minimal and add commands as you need them. The REPL shape lets an agent run it inside tmux and iterate without relaunching the (slow) app on every interaction.
...
This is a starting skeleton. As you reach interesting parts of the app you'll add app-specific commands — navigate to a view, focus a weird input, bypass an auth gate. Those commands encode hard-won knowledge; keep them.

## Step 3 — use it yourself, via tmux

Run the driver the way the next agent will:
...
Then open \`/tmp/shots/01-landing.png\`. Is it the app, blank, or a login screen? Each tells you what to do next. Click into the main feature, fill a form, see the result, screenshot it. The driver grows whatever commands you need (\`focus-input\`, \`goto-settings\`, \`login-as-test-user\`…). When one real flow works end-to-end, you're ready to write.

## Step 4 — write SKILL.md

Short. The driver is the meat; \`SKILL.md\` is the manual. Structure that works:
...
Real patterns from real Electron apps — you'll hit some subset:

- **\`firstWindow()\` gives you a splash/loading screen,** not the app. Wait longer, find the right page by URL, or wait for a selector that only appears when the app is actually ready.

- **The real UI is in a BrowserView, not a BrowserWindow.** Playwright sees it as a separate "window" with a different URL. The \`windows\` command exists for this. \`getBrowserViews()\` may return empty on newer Electron — use \`webContents.getAllWebContents()\` instead.

- **\`locator.click()\` clicks the wrong thing.** Playwright computes click coordinates relative to the main window; if your content is in a BrowserView overlay, those coordinates hit the window behind it. The skeleton uses \`page.evaluate(el => el.click())\` to bypass coordinates entirely.

- **Feature gates block what you need to test.** The app checks a plan tier, an env flag, or a flag baked into SSR HTML. Find where the check happens (grep the built output for the gate name) and patch it for your local run — a \`sed\` on the build output, an env var override, or (for SSR-embedded flags) intercept the response via CDP \`Fetch.enable\` and rewrite it in-flight. Document exactly what you patched and why.
