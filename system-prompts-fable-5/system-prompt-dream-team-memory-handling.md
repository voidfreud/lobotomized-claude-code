...
- **Phase 3:** If a personal memory restates a team memory, delete the personal one. Merge near-duplicates *within* `team/` the same way you would personal memories.
- **Phase 4 — be conservative pruning `team/`:** Delete or fix a team memory clearly contradicted by the current code, or superseded by a newer one. When unsure, leave it. A stale team memory costs little; deleting a teammate's load-bearing note costs a lot.
