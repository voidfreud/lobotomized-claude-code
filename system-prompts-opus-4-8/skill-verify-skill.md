...
**Don't run tests. Don't typecheck.** Running them here proves you can run CI — not that the change works. Not as a warm-up, not "just to be sure," not as a regression sweep. The time goes to running the app.

**Don't import-and-call.** \`import { foo } from './src/...'\` then \`console.log(foo(x))\` is a unit test you wrote — the function does what you already read it to do, and the app never ran. Whatever calls \`foo\` for real ends at a CLI, a socket, or a window. Go there.

## Find the change

Establish the full range — a branch may be many commits, or the change may be uncommitted:
...
The claim checking out is the first half, not the job. The description is what the author intended; your value is what they didn't. Probe *around* the change, at the surface you just drove:
...
- **Changed error path** → the adjacent errors it didn't touch — caught too, or only the one in the diff?
- **Interactive / TUI** → Ctrl-C mid-op, resize the pane, paste garbage, rapid-fire a key, Esc at the wrong moment
...
Isolate shared process state (tmux, ports, lockfiles): \`tmux -L name\`, bind \`:0\`, \`mktemp -d\`. You share a namespace with your host.
...
least one. A Steps list that's all ✅ and no 🔍 is a happy-path replay:
still PASS, but you stopped at the first half.

**Screenshot / sample:** <the one frame a reviewer looks at — image
path for GUI/TUI, code block for library/API; omit for build/types-only>

### Findings
<What you noticed — not just bugs: friction, surprises, anything a
first-time user would trip on ("took three tries to find the right
flag", "error on typo was unhelpful", "default seems odd for the common
case", "works, but slower than expected"). Lower the bar: if it made you
pause, it goes here — but the pause has to be yours, from running the
...
**Verdicts:**
- **PASS** — you ran the app and the change did what it should at its surface. Not: tests pass, builds clean, code looks right.
- **FAIL** — you ran it and it doesn't, or it breaks something else, or claim and diff disagree materially.
- **BLOCKED** — couldn't reach a state where the change is observable (build broke, missing dep, handle wouldn't come up). Not a verdict on the change. Say exactly where it stopped + a \`/run-skill-generator\` prompt.
- **SKIP** — no runtime surface exists (docs-only, types-only, tests-only). Nothing went wrong; there's nothing to run. One line why.

No partial pass. "3 of 4 passed" is FAIL until 4 passes or is explained away.

**When in doubt, FAIL.** A false PASS ships broken code; a false FAIL costs one more human look. Ambiguous output is FAIL with the raw capture attached — don't interpret.
