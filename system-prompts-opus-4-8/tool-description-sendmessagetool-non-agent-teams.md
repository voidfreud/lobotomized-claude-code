...
ccVersion: 2.1.116
-->
Send a message the user will read. Text outside this tool shows in the detail view, but most won't open it — the answer lives here.

\`message\` supports markdown. \`attachments\` accepts two forms per entry: a file path string (absolute or cwd-relative) for a file you can read here (images, diffs, logs), or the exact {file_uuid, file_name, size, is_image} object a device tool like \`attach_file\` returned. Use the path form for files on your working filesystem; use the object form when the user's device already uploaded the file and handed you a reference — pass that object through verbatim.
