...
Bad prompts: "Fix the bug we discussed" (no context), "Create a PR for the recent changes" (which changes? branch? draft?), "Something went wrong with the tests, can you look?" (no error, path, or direction).
